...
fn abe() -> Result<(), FormatErr> {
    ...
    // hierarchical axis made of 5 levels: Protected, Low_Secret , ...,
    // Top_Secret. It is hierarchical: a user being granted access to level `n`
    ...
    // The second Policy Axis is the Department axis and is made of 4 values: RnD,
    ...
    // to 2 Delegates:
    //  - a "High_Secret Marketing and Finance Delegate" which can only generate
    //    User Keys for marketing (MKG) and/or finance (FIN) data of Security Level
    //    High_Secret and below
    let high_secret_fin_mkg_access_policy = AccessPolicy::from_boolean_expression(
        "Security_Level::High_Secret & (Department::MKG | Department::FIN)",
    )?;
    ...
    // their policy. A marketing user with Medium_Secret security can have
    // its key generated by any of the Delegates.
    let medium_secret_mkg_access_policy =
        AccessPolicy::from_boolean_expression("Security_Level::Medium_Secret & Department::MKG")?;
    ...
    let top_secret_mkg_access_policy =
        AccessPolicy::from_boolean_expression("Security_Level::Top_Secret & Department::MKG")?;
    ...
    // FAILURE: as expected the High_Secret marketing authority cannot generate user
    // keys for the Top_Secret Security Level
    assert!(top_secret_user.is_err());

    let medium_secret_hr_access_policy =
        AccessPolicy::from_boolean_expression("Security_Level::Medium_Secret & Department::HR")?;
    ...
    // ### A Low_Secret marketing message
    // Let us create an encrypted marketing message with a Low_Secret level
    let low_secret_mkg_message = engine.random_message()?;
    let low_secret_mkg_cipher_text = engine.encrypt(
        &policy,
        &public_key,
        &[
            ("Security_Level", "Low_Secret").into(),
            ("Department", "MKG").into(),
        ],
    ...
    // ### A Top_Secret marketing message
    // However in the case of a Top_Secret marketing message, only the super user
    ...
    let top_secret_mkg_cipher_text = engine.encrypt(
        ...
        &public_key,
        &[
            ("Security_Level", "Top_Secret").into(),
            ("Department", "MKG").into(),
        ],
    ...
    // ### A Low_Secret HR message
    // Likewise, in the case of a Low_Secret HR message, only the super
    ...
    // attribute. Let us rotate the Security_Level Low_Secret
    policy.rotate(&("Security_Level", "Low_Secret").into())?;

    // We now encrypt a new marketing message at (the new) Low_Secret level
