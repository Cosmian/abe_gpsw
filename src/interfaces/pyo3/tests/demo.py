...
policy = cosmian_abe_gpsw.generate_policy(
    policy_axis_bytes=policy_axis, max_attribute_creations=100)

master_keys = cosmian_abe_gpsw.generate_master_keys(policy)

top_secret_mkg_fin_user = cosmian_abe_gpsw.generate_user_private_key(
    master_keys[0], "Security Level::Top Secret && (Department::MKG || Department::FIN)", policy)

medium_secret_mkg_user = cosmian_abe_gpsw.generate_user_private_key(
...
low_secret_mkg_data = cosmian_abe_gpsw.encrypt(metadata, policy, bytes(json.dumps(
    ['Security Level::Low Secret', 'Department::MKG']), 'utf8'), master_keys[1], plaintext_bytes)
top_secret_mkg_data = cosmian_abe_gpsw.encrypt(metadata, policy, bytes(json.dumps(
    ['Security Level::Top Secret', 'Department::MKG']), 'utf8'), master_keys[1], plaintext_bytes)
low_secret_fin_data = cosmian_abe_gpsw.encrypt(metadata, policy, bytes(json.dumps(
...
cleartext = cosmian_abe_gpsw.decrypt(top_secret_mkg_fin_user, low_secret_mkg_data)
assert(str(bytes(cleartext), "utf-8") == plaintext)

cleartext = cosmian_abe_gpsw.decrypt(top_secret_mkg_fin_user, top_secret_mkg_data)
assert(str(bytes(cleartext), "utf-8") == plaintext)

cleartext = cosmian_abe_gpsw.decrypt(top_secret_mkg_fin_user, low_secret_fin_data)
assert(str(bytes(cleartext), "utf-8") == plaintext)
...
new_policy = cosmian_abe_gpsw.rotate_attributes(bytes(json.dumps(
    ['Security Level::Low Secret']), 'utf8'), policy)
# print(new_policy)
new_low_secret_mkg_data = cosmian_abe_gpsw.encrypt(metadata, new_policy, bytes(json.dumps(
    ['Security Level::Low Secret', 'Department::MKG']), 'utf8'), master_keys[1], plaintext_bytes)
...
new_medium_secret_mkg_user = cosmian_abe_gpsw.generate_user_private_key(
...
super_delegate = cosmian_abe_gpsw.generate_delegated_key(
